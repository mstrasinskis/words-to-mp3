...
const buildTranslationPrefixRule = ({ langTo, langBeforeTranslation }) => {
  if (!langBeforeTranslation) {
    return `Keep the translations clearly separated and in this order: ${formatHumanList(langTo)}.`;
  }

  const translationPrefixes = parseCommaSeparatedValues(langBeforeTranslation);

  if (translationPrefixes.length === 1 && langTo.length === 1) {
    return `Insert the "${translationPrefixes[0]}: " right before the translation.`;
  }

  if (translationPrefixes.length === langTo.length) {
    const prefixInstructions = langTo.map(
      (targetLanguage, index) =>
        `"${translationPrefixes[index]}: " before the ${targetLanguage} translation`
    );

    return `Insert ${formatHumanList(prefixInstructions)}. Keep the translations in that order.`;
  }
...
const buildPrompt = ({
  ...
  const languagePairs = langTo.map(
    (targetLanguage) => `${langFrom}-${targetLanguage}`
  );
  const translationPrefixRule = buildTranslationPrefixRule({
    langTo,
    langBeforeTranslation,
  });
  const translationLayoutRule =
    langTo.length > 1
      ? 'Include one translation per target language in that same first block, separated by "<br>".'
      : "";

  return `
I am preparing ${formatHumanList(languagePairs)} vocabulary (${langContext}) for study.
...
1. Translation into ${formatHumanList(langTo)}. ${translationPrefixRule} ${translationLayoutRule}
...
- Important: At least two blocks on the second page must be separated by an "<hr>" tag.
...
export const wordsToTsv = async (apiKey, inputPath, outputPath) => {
  ...
  const langTo = parseCommaSeparatedValues(process.env.LANG_TO || "");
  ...
  const inputLines = parseInputEntries(raw)
